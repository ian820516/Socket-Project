...
int main(int argc, char* argv[]) {
    ...
    memset(&hospA, 0, sizeof(hospA));
    hospA.ai_family = AF_UNSPEC;
    hospA.ai_socktype = SOCK_DGRAM;
    hospA.ai_flags = AI_PASSIVE;

    if ((status = getaddrinfo(HOSPIADDR, HOSPITALA_UDP_PORT, &hospA, &res)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(status));
        return 1;
    }
    ...
    if (p == NULL) {
        fprintf(stderr, "HOSPA: failed to connect\n");
        return 2;
    }

    inet_ntop(p->ai_family, get_in_addr((struct sockaddr*)p->ai_addr), s, sizeof(s));
    //fprintf(stdout, "HostA: connecting to %s\n", s);
    freeaddrinfo(res);

    addr_len = sizeof(their_addr);
    
    fprintf(stdout, "Hospital A is up and running using UDP on port %s.\n", HOSPITALA_UDP_PORT);
    fprintf(stdout, "Hospital A has total capacity %d and initial occupancy %d\n", msg.capcity, msg.occupancy);
    ...
    while(true) {
        ...
        if ((numbytes = recvfrom(sockfd, client_loc, sizeof(client_loc), 0, (struct sockaddr*)&their_addr, &addr_len)) == -1) {
            perror("Recv from Host A");
            exit(1);
        }

        fprintf(stdout, "Hospital A has received input from client at location %s\n", client_loc);
        ...
        if (!map_node.count(location)) {
            fprintf(stdout, "Hospital A does not have the location %d in map\n", location);
            fprintf(stdout, "Hospital A has sent \"location not found\" to the scheduler\n");
            distance = -1; 
        } 

        fprintf(stdout, "Hospital A has capacity = %d, occupation = %d,", msg.capcity, msg.occupancy);
        if (availabilty == -1) {
        ...
        // print disrtance msg
        if (distance == -1) {
            fprintf(stdout, "Hospital A has found the shortest path to client, distance = NONE\n");
            msgA.distance = -1;
        } else {
            fprintf(stdout, "Hospital A has found the shortest path to client, distance = %.6f\n", distance);
            msgA.distance = distance;
        }

        if (distance == -1 || availabilty == -1) {
            score = -1; // NONE
            msgA.score = -1;
            fprintf(stdout, "Hospital A has the score = NONE\n");
        } else {
            score = (double)1 / (distance*(1.1 - availabilty));
            fprintf(stdout, "Hospital A has the score = %.6f\n", score);
            msgA.score = score;
        }
        // send score and distance to scheduler
        
        if ((numbytes = sendto(sockfd, &msgA, sizeof(CalMSG), 0, (struct sockaddr*)&their_addr, addr_len)) == -1) {
            perror("HostA Score and Distance: sendto");
            exit(1);
        }

        if (distance == -1 && score == -1) {
            fprintf(stdout, "Hospital A has sent score = NONE and distance = NONE to the scheduler\n");
        } else if (distance == -1) {
            fprintf(stdout, "Hospital A has sent score = %.6f and distance = NONE to the scheduler\n", score);
        } else if (score == -1) {
            fprintf(stdout, "Hospital A has sent score = NONE and distance = %.6f to the scheduler\n", distance);
        } else {
            fprintf(stdout, "Hospital A has sent score = %.6f and distance = %.6f to the scheduler\n", score, distance);
        }
        ...
        if (assign == 1) {
            msg.occupancy += 1;
            availabilty = (double)(msg.capcity - msg.occupancy) / msg.capcity;
            fprintf(stdout, "Hospital A has been assigned to a client, occupation is update to %d, availabiltiy is updated to %.6f\n", msg.occupancy, availabilty);
        }
        
        if ((numbytes = sendto(sockfd, &msg, sizeof(HospMsg), 0, (struct sockaddr*)&their_addr, addr_len)) == -1) {
            perror("HostA Update: sendto");
            exit(1);
        }
        fprintf(stdout, "------------------------------------------------------------------\n");
    }
